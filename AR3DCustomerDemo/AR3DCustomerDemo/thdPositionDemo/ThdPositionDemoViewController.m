//
//  ThdPositionDemoViewController.m
//  AR3DCustomerDemo
//
//  Created by muxiaohui on 2017/11/13.
//  Copyright © 2017年 lymuxh. All rights reserved.
//

#import "ThdPositionDemoViewController.h"

@interface ThdPositionDemoViewController ()
...
@implementation ThdPositionDemoViewController
...
-(void)initplane{
    ...
    //2.使用Material渲染3D模型
    earth.firstMaterial.diffuse.contents = @"earth.scnassets/earth/earth-diffuse-mini.jpg";;
    earth.firstMaterial.emission.contents = @"earth.scnassets/earth/earth-emissive-mini.jpg";
    //3.创建一个基于3D物体模型的节点
    SCNNode *earthNode = [SCNNode nodeWithGeometry:earth];
    //4.设置节点的位置 SceneKit框架中节点的位置position是一个基于3D坐标系的矢量坐标SCNVector3Make
    earthNode.position =SCNVector3Make(5, 5, -5);
    ...
    //4.设置节点的位置为捕捉到的平地的锚点的中心位置 SceneKit框架中节点的位置position是一个基于3D坐标系的矢量坐标SCNVector3Make
     moonNode.position =SCNVector3Make(-5,5,-5);
    
    //月球添加到地球子节点
    [earthNode addChildNode:moonNode];
    
    
    SCNSphere *moonplane2 = [SCNSphere sphereWithRadius:0.5];
    moonplane2.firstMaterial.diffuse.contents =[UIColor redColor];
    SCNNode *moonNode2 = [SCNNode nodeWithGeometry:moonplane2];
    moonNode2.position = SCNVector3Make(-5, 5, -5);
    
    [self.arSCNView.scene.rootNode addChildNode:earthNode];
    [self.arSCNView.scene.rootNode addChildNode:moonNode2];
