...
@interface thdVideoDemoViewController ()
//AR视图：展示3D界面
...
@implementation thdVideoDemoViewController
...
-(void)initplane{
    ...
    //1.创建一个3D物体模型
    SCNSphere *panorama = [SCNSphere sphereWithRadius:100];
    ...
    panoramaNode.rotation =SCNVector4Make(0, 1, 0, M_PI);
    [self.arSCNView.scene.rootNode addChildNode:panoramaNode];
    
    
    NSURL *url= [[NSBundle mainBundle] URLForResource:@"960p" withExtension:@"mp4"];
    ...
    scene.size = videoNode.size;
    panorama.firstMaterial.diffuse.contents = scene;
